...
def LaTeX2HTML(archivo):
    ...
    texto = re.sub(
        r"<strong>Problema \d+</strong>", r"<strong>Problema</strong>", texto
    )
...
def insertar_regla_ip(ip, headers, url, tipo="whitelist", comentario="Script"):
    ...
    if respuesta.get("success"):
        print(f"Regla de acceso para IP {ip} insertada exitosamente.")
...
def eliminar_regla_ip(identificador_regla, headers, zona_id):
    ...
    if respuesta.get("success"):
        print(f"Regla de acceso con ID {identificador_regla} eliminada exitosamente.")
        return True
    else:
...
if __name__ == "__main__":
    ...
    if ip_publica:
        identificador_regla = insertar_regla_ip(
            ip_publica,
            headers_clouflare,
            url_cloudflare,
            tipo="whitelist",
            comentario="Github Actions",
        )
